...
function findVideoAndAddListeners() {
    ...
    if (videoElement) {
        ...
        // Add our synchronized event listeners
        ...
        // Create a MutationObserver to watch for player changes
        setupPlayerObserver();
        
        // Apply control restrictions based on current permissions
        applyVideoControlRestrictions();
...
function onYouTubeNavigate() {
    // Re-find the video element as it might have changed
    videoElement = document.querySelector('.html5-main-video');
    
    // Update title and reapply restrictions
    updateVideoTitle();
    applyVideoControlRestrictions();
}

function setupPlayerObserver() {
    // Observer to detect changes to the player that might affect our controls
    const observer = new MutationObserver(mutations => {
        // Re-apply restrictions when player changes
        applyVideoControlRestrictions();
    });
...
function resetPartyState() {
    ...
    // Restore YouTube controls to normal
    removeVideoControlBlocker();
    disableYouTubeControls(); // This will restore controls since canControl is false
}
...
function applyVideoControlRestrictions() {
    ...
    // Call our new function to disable YouTube controls
    ...
    // Add or remove the physical blocker
...
function addVideoControlBlocker() {
    ...
    // Create blocker element - cover the ENTIRE player, not just controls
    ...
    `;
    message.textContent = "Only the host can control playback";
    ...
    // Add click handler to ensure we cancel all clicks
...
// NEW: Disable YouTube controls when not allowed
function disableYouTubeControls() {
    // Only disable if we're in a party and don't have control
    if (hostPeerId && !isHost && !canControl) {
        // Find YouTube's control elements and disable them
        const controlElements = document.querySelectorAll('.ytp-chrome-bottom, .ytp-chrome-controls');
        controlElements.forEach(element => {
            element.style.pointerEvents = 'none';
            element.style.opacity = '0.5';
        });
        
        // Set video element properties to block default controls
        if (videoElement) {
            // Store the original state to restore later
            videoElement.dataset.originalControlsState = videoElement.controls;
            videoElement.controls = false;
        }
    } else {
        // Restore controls if we have permission
        const controlElements = document.querySelectorAll('.ytp-chrome-bottom, .ytp-chrome-controls');
        controlElements.forEach(element => {
        ...
        // Restore video element's original state
        if (videoElement && videoElement.dataset.originalControlsState !== undefined) {
            videoElement.controls = (videoElement.dataset.originalControlsState === "true");
        }
    }
}
